    ...
    while (train_time := time.time() - train_start) / 60 < args.train_time_minutes:
        ...
        for optim, max_lr in optimizers.items():
            for param_group in optim.param_groups:
                param_group["lr"] = get_lr(step, max_lr)
            optim.step()
        
        tok_s = args.micro_batch_size * args.seq_len * args.grad_accum_steps / (time.time() - step_t0)
        run.log({
            "step": step,
            "loss": loss.item(),
            "time": train_time,
            "norm": norm.item(),
            "tok/s": tok_s,
            "lr": get_lr(step, args.muon_lr),
        })

        if step % args.log_every == 0:
            print(
                f"step: {step:8d} | loss: {loss.item():8.4f} | norm: {norm.item():8.4f} | time: {train_time:8.2f} | tok/s: {tok_s:8f} | lr: {get_lr(step, args.muon_lr):8.6f}"
            )
